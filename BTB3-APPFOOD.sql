-- Lưu ý Bài làm ở phía dưới data
...
SELECT users.user_id, users.full_name, COUNT(like_res.like_res_id) AS total_likes
FROM users
...
SELECT restaurant.res_id, restaurant.res_name, COUNT(like_res.like_res_id) AS total_likes
FROM restaurant
...
SELECT users.user_id, users.full_name, COUNT(orders.order_id) AS total_orders
FROM users
